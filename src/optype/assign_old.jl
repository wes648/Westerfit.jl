...
"""
vt2m(vtm)::Int = ceil(Int,vtm/2)*powneg1(vtm)
...
kperm2(n::Int)::Array{Int} = sortperm(powneg1.(collect(-n:n).+iseven(n)) .* collect(-n:n))
...
function assign_big_call(ctrl::Controls,wvs::Eigs,ψ::Psi,σid::Int)::Eigs
   if ctrl.stagess==1
      wvs.rst.vals, wvs.rst.vecs = assign_1stg!(ctrl, wvs.rst.vals, wvs.rst.vecs,ψ)
   elseif
      perm = twostg_assign(wvs.rst.vecs,ψ.R.J,ψ.R.S,md,vtmax)
      wvs.rst.vals[perm,σid] .= wvs.rst.vals[perm,σid]
      wvs.rst.vecs[:,:,σid]  .= wvs.rst.vecs[:,perm,σid]
   else
      @warn "Stage number $(ctrl.stages) not implemented"
   end
   return wvs
end
