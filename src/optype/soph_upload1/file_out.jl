...
function reslin(qns,ofrq,omc,cfrq)       #this function does the pretty formatting for the
   part  = lpad(Int(0.5*qns[ 1]),4)*";"      #reswriter function with a bunch of lpads
   part *= lpad(Int(qns[ 2]),3)*";"
   part *= lpad(Int(qns[ 3]),3)*";"
   part *= lpad(Int(qns[ 4]),3)*";"
   part *= lpad(Int(qns[ 5]),3)*";"
   part *= lpad(Int(0.5*qns[ 6]),4)*";"
   part *= lpad(Int(qns[ 7]),3)*";"
...
function reswritter(molnam::String,qns,lines::Lines,omcs,cfrqs)  #writes the .res file
   ...
   io = open("$molnam.res", "w") do io
      for i in 1:len
         println(io, reslin(qns, lines.frqs[i], omcs[i], cfrqs[i]))
      end                   
   end
end
...
function output_init(molnam::String,ctrl::Controls,ℋ::Vector{Term})
   ...
   println(io, "\nInitial Parameters")
   ham_init_print(io, ℋ)
   sum_init_print(io, nparam, lins, omc)
   close(io)
   end
end
function ham_print(io,ℋ::Vector{Op},δ::Vector{Float64},prjct::Vector{Int})
   j = 1
