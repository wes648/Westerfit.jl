...
function Hr0K(pr,N,K)
   #<N K |Hrot| N K>
   #   out = @. (0.5*(pr[2] + pr[3])+ pr[13]*N*(N+1.0))*N*(N+1.0) + (pr[1] - 0.5*(pr[2] + pr[3]))*K^2
   out = @. (pr[2] + (pr[16]+ pr[45]*K^2 +pr[44]*N*(N+1.0))*N*(N+1.0))*N*(N+1.0)
   out = @. out + (pr[1] + (pr[18]+ pr[46]*N*(N+1.0) +pr[47]*K^2)*K^2 + pr[17]*N*(N+1.0))*K^2
   return out
end
function Hr1K(pr,N,K)
   #<N K+1 |Hrot|N K>
   out = @. pr[4]*fh(N,K)*(K+0.5)
end
function Hr2K(pr,N,K)
   #<N K+2 |Hrot| N K>
   out = @. (pr[19]+pr[48]*N*(N+1.0))*N*(N+1.0)
   out = @. out + (pr[20] + pr[49]*N*(N+1.0))*(K^2 + (K+2)^2) + pr[50]*(K^4 + (K+2)^4)
   out = @. (out+pr[3])*fh(N,K)*fh(N,K+1.0)
end
#=function Hrot(pr,N)
   if N == zero(N)
...
function Hrot(pr,N)
   if N == zero(N)
      ...
      @inbounds of1diag = Hr1K(pr,N,karray[1:end-1])
      @inbounds of2diag = Hr2K(pr,N,karray[1:end-2])
...
function Hrt0K0m(pr,nt,K,m,σ)
   ...
   out = @. out + pr[26]*nt + (pr[27]+pr[31])*K^2 + pr[23]
   out = @. out*m^2 + (-2.0*pr[6]+pr[25]+pr[34]*nt+pr[35]*K^2)*m*K
   out = @. out + pr[30]*nt + pr[7]*0.5 + pr[22]*0.5
end
function Hrt1K0m(pr,N,K,m,σ) #not correct
   #<N K+1 m+1 | H |N K>
   out = @. (pr[32]+pr[29]*m^2)*(K+0.5)
   out = @. out + pr[36]*m*(K^2 + (K+1.0)^2) + pr[37]*0.5*m*N*(N+1.0)
   out = @. out*fh(N,K)
end
function Hrt2K0m(pr,N,K,m,σ)
   out = @. pr[30]+pr[28]*m^2 + 0.0*m*K
   out = @. 0.5*out*fh(N,K)*fh(N,K+1.0)
end
function Hrt0K1m(pr,nt,K,m,σ) #not correct
   #<m+1 K|Htor|m K> = -V3/4
   out = @. -pr[25]*K*(m-0.5*NFOLD) - pr[30]*nt
   out = @. out - 0.5*(pr[7]*0.5 + pr[23]*(m^2 +(m-NFOLD)^2) + pr[31]*K^2)
end
function Hrt1K1m(pr,N,K,m,σ)
   #<N K+1 m+1 | H |N K>
   out = @. -0.5*pr[32]*fh(N,K)*(K+0.5) + 0.0*m*K
end
function Hrt2K1m(pr,N,K,m,σ)
   out = @. -0.25*pr[33] + 0.0*m*K
   out = @. out*fh(N,K)*fh(N,K+1.0)
end
function Hrt0K2m(pr,N,K,m,σ)
   #<m+1 K|Htor|m K> = -V3/4
...
function Htor(pr,mcalc,N,σ)
   ...
   ms = kron(msbuilder(Float64,mcalc,σ,NFOLD), ones(Float64,tnp))
   t0k0m = Hrt0K0m(pr,N*(N+1.0),ks,ms,σ)
   t1k0m = Hrt1K0m(pr,N,ks[1:end-1],ms[1:end-1],σ) #not correct
   t2k0m = Hrt2K0m(pr,N,ks[1:end-2],ms[1:end-2],σ)
   t0k1m = Hrt0K1m(pr,N*(N+1.0),ks[tnp+1:end],ms[tnp+1:end],σ) #not correct
   t1k1m = Hrt1K1m(pr,N,ks[tnp:end],ms[tnp:end],σ)
   t2k1m = Hrt2K1m(pr,N,ks[2*N:end],ms[2*N:end],σ)
   t0k2m = Hrt0K2m(pr,N,ks[2*tnp+1:end],ms[2*tnp+1:end],σ)
   #overlapping bands:
   if N==one(N)
   t1k0m .+= t2k1m
   t2k0m .+= t1k1m
...
#   <J S N K| Hsr |J S N K>
   ...
   out = @. thet*(pr[9]*(3.0*K^2 - nt2) + pr[41]*K^4 + (pr[39] + pr[40])*nt2*K^2 + pr[38]*nt2^2)
   out = @. out - 0.5*pr[8]*(J*(J+1.0)-nt2-S*(S+1.0))
end
...
function Htsrmat2(pr,hrsr,j,s,mcalc,σ)
   ...
   marray = msbuilder(Float64,mcalc,σ,NFOLD)
   tspart = kron(diagm(0=>marray),tspart)
   out = kron(eye(2*mcalc+1),Spicall(j,s,hsrs)) + tspart + Htor(pr,mcalc,j,s,σ)
   return out
end
